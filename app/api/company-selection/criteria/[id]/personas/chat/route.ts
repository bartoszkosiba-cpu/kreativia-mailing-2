...
function buildPayload(
  ...
  return {
    name: overrides.name ?? existing?.name ?? `Persony #${criteriaId}`,
...
export async function POST(
  ...
  try {
    ...
    const existing = await getPersonaCriteria(criteriaId);
    const baseCriteria = await db.companyVerificationCriteria.findUnique({ where: { id: criteriaId } });
...
Opis: ${baseCriteria?.description ?? "brak"}`;
    ...
    const payload = buildPayload(criteriaId, existing, {
      name: existing?.name ?? baseCriteria?.name ?? `Persony #${criteriaId}`,
      description: existing?.description ?? baseCriteria?.description ?? undefined,
      chatHistory,
    ...
    const saved = await upsertPersonaCriteria(criteriaId, payload);

    logger.info("persona-criteria-chat", "Zapisano wiadomość w czacie person", { criteriaId });
    ...
    logger.error("persona-criteria-chat", "Błąd rozmowy o personach", { criteriaId }, err);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    if (!existing) {
      return NextResponse.json(
        { success: false, error: "Brak historii rozmowy o personach" },
        { status: 404 }
      );
    }
...
${JSON.stringify(history, null, 2)}`;
    ...
    logger.info("persona-criteria-generate", "Zaktualizowano konfigurację person", { criteriaId });

    return NextResponse.json({ success: true, data: saved });
  } catch (error) {
    const err = error instanceof Error ? error : new Error(String(error));
    logger.error("persona-criteria-generate", "Błąd generowania person", { criteriaId }, err);
    return NextResponse.json(
